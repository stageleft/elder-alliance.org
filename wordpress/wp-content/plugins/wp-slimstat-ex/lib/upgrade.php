...
<?php
	...
	function remove_unuesed_indexkeys_before_16_2() {
		...
		if(!$upgrade) return false;
		$upgrade = $ssAdmin->maybe_remove_indexkey('ip_from_to_idx', 'country');
		if(!$upgrade) return false;
		$upgrade = $ssAdmin->maybe_remove_indexkey('ip_from_idx', 'country');// wp-slimstat 0.92
		return $upgrade;
	}
...
//					echo 'affected rows : '. $affected. ' ('.$r->id.' - '. $r->rs_string.')<br />'. "\n";
	...
	function upgrade_from_before_16($step, $message, $buttontext) {
		...
		switch($substep) {
			...
			break;
			case 21:
				$done = $ssAdmin->maybe_add_indexkey('ip_to', 'country');
			break;
			case 22:
				$done = $ssAdmin->pack_indexkey('country', '1');
			break;
			case 23:
	...
	function do_upgrade($step=1) {
		...
		if(!$current_version) {
			$updated = wp_slimstat_ex::check_current_version();
			if($updated)
				return true;
			$current_version = get_option('wp_slimstat_ex_version');
		}
