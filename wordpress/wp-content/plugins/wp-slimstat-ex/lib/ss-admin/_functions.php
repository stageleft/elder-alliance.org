<?php 
if(!class_exists('wp_slimstat_ex')) die('Can not find Wp-SlimStat. Please activate plugin');
...
<?php 
	...
	function set_dt_data($custom ='', $type = 0) {
		...
		$max_age = $this->dt[0] - ( $custom * 86400 );
		$max_age_svr = $this->dt[1] - ( $custom * 86400 );
		$first_hit = SSFunction::get_firsthit($typeN);
		if ( $max_age_svr > $first_hit ) {
//			set_time_limit(120);// increase time limit of PHP execution
				print "<p>Inserting ";
				$dt = strtotime( date( "Y-m-d 00:00:00", $this->sstime($first_hit) ) );
				while ( $dt < $max_age ) {
					// hour
					unset($dt_svr);
					unset($dt_end);
...
//					endif;
			...
			?>
			<p><?php _e('The oldest entry in the database is from', 'slimstat-admin'); ?> <?php print date( "j M Y", $this->sstime(SSFunction::get_firsthit()) ); ?>.</p>
			<p><?php _e('SlimStat is configured to delete data more than', 'slimstat-admin'); ?> <?php print $this->option['dbmaxage']; ?> <?php _e('days old, or from before', 'slimstat-admin'); ?> <?php print date( "j M Y", $max_age ); ?>.</p>
...
//		print_r($current);return;
		...
		$table .= "<th>Column</th>";
		$table .= "<th style=\"text-align: left;\">Related</th>";
		$table .= "<th>Action</th>\n";
		$table .= "</tr>\n";
	...
	function _do_indexing($job, $key, $table, $echo=true, $index_type="INDEX") {
		...
		if( $job == "" || $key == "" || $table == "" ) {
			if($echo)
				echo '<p class="message">'.__('Required values are not defined', 'slimstat-admin').'</p>';
			return false;
		}
		$column_array = array( 'dt_total' => '(remote_ip, dt, visit)', 'resource_total' => '(resource)' );
		...
		switch($job) {
			case 'add':
				if(isset($current[$key])) {
					if($echo)
						echo '<p class="message red">'.sprintf(__('"%s" key already exists', 'slimstat-admin'), $key).'</p>';
					return true;
				}
			...
			case 'remove':
				if(!isset($current[$key])) {
					if($echo)
						echo '<p class="message red">'.sprintf(__('"%s" key does not exists', 'slimstat-admin'), $key).'</p>';
					return true;
				}
			...
			default:
				if($echo)
					echo '<p class="message red">'.__('job(add,remove,readd) is not defined', 'slimstat-admin').'</p>';
				return false;
			break;
		}
		...
		if($echo) {
			if($do_indexing === false) {
				echo '<p class="message red">'.$msg['fail'].'</p>';
			} else {
				echo '<p class="message blue">'.$msg['success'].'</p>';
			}
		} else {
	...
	function check_country_file($myCountryFile = 'ip-to-country.csv') {
		global $wpdb;
//		$myCountryFile = "ip-to-country.csv";
		$isCountryFile = false;
		$myFileHandle = opendir(dirname(__FILE__));
	...
	function _importCountriesDataN($myCountryFile = 'ip-to-country.csv') {
		global $wpdb;
...
//		$myCountryFile = "ip-to-country.csv";
		...
		if ( $isCountryFile ) {
			...
			// Remove all the rows
			$myQuery = "TRUNCATE $this->table_countries";
			
			if ( $wpdb->query( $myQuery ) === false ) {
				return false;
			}

			if($file = file(dirname(__FILE__) . '/'.$myCountryFile)) {
				for ($line=0; $line<count($file); $line++) {
					switch($myCountryFile) {
						case 'ip-to-country.csv':
						list($ipfrom,$ipto,$countrycode2,$countrycode3,$country) = explode(',',addslashes(eregi_replace('"','',trim($file[$line]))), 5);
						break;
						case 'GeoIPCountryWhois.csv':
						list($ip1,$ip2,$ipfrom,$ipto,$countrycode2,$country) = explode(',',addslashes(eregi_replace('"','',trim($file[$line]))));
						break;
						default:break;
					}
					$query = "INSERT INTO $this->table_countries 
							  (ip_from,ip_to,country_code) VALUES
							  ($ipfrom,$ipto,'".strtolower($countrycode2)."')";
	...
	function update_country_data($table = 'common') {
		...
		$_where = "country NOT IN ('gb', 'us', 'ca', 'mx', 'bm', 'se', 'it', 'a2', 'ch', 'pr', 'in', 'vi', 'de', 'ir', 'bo', 'ng', 'nl', 'fr', 'il', 'es', 'pa', 'cl', 'bs', 'ar', 'dm', 'be', 'ie', 'bz', 'br', 'za', 'sc', 'tz', 'eg', 'rw', 'dz', 'sz', 'gh', 'cm', 'mg', 'ke', 'ao', 'na', 'ma', 'sl', 'ci', 'ga', 'mu', 'tg', 'ly', 'sn', 'sd', 'ug', 'zw', 'mz', 'zm', 'bw', 'lr', 'cd', 'bj', 'tn', 'jp', 'eu', 'au', 'th', 'cn', 'my', 'pk', 'nz', 'kr', 'hk', 'sg', 'bd', 'id', 'ph', 'tw', 'af', 'vn', 'vu', 'nc', 'bn', 'ap', 'gr', 'sa', 'pl', 'cz', 'ru', 'cy', 'no', 'at', 'ua', 'tj', 'dk', 'pt', 'tr', 'ge', 'by', 'iq', 'am', 'lb', 'md', 'bg', 'fi', 'om', 'lv', 'kz', 'ee', 'sk', 'jo', 'hu', 'kw', 'al', 'lt', 'sm', 'bt', 'ro', 'rs', 'hr', 'lu', 'is', 'li', 'cr', 'mk', 'mt', 'gm', 'mw', 'si', 'fk', 'az', 'mc', 'ht', 'gu', 'jm', 'fm', 'ec', 'co', 'pe', 'ky', 'gp', 'hn', 'ye', 'vg', 'lc', 'sy', 'ni', 'do', 'an', 'gt', 've', 'ba', 'hm', 'uy', 'sv', 'ae', 'tt', 'lk', 'bv', 'mh', 'bh', 'ck', 'gi', 'py', 'ag', 'ls', 'kn', 'ws', 'pw', 'qa', 'kh', 'ai', 'as', 'tc', 'mp', 'uz', 'mo', 'um', 're', 'gy', 'cu', 'cg', 'a1', 'bb', 'la', 'sr', 'aw', 'fj', 'ms', 'gd', 'vc', 'np', 'kg', 'me', 'td', 'fo', 'so', 'ml', 'ps', 'bi', 'gn', 'et', 'km', 'mr', 'mq', 'ne', 'va', 'tm', 'yt', 'bf', 'ad', 'aq', 'gl', 'wf', 'pg', 'mn', 'pf', 'mv', 'cf', 'er', 'gw', 'dj', 'gq', 'cv', 'st', 'gf', 'sb', 'tv', 'ki', 'to', 'io', 'nu', 'tk', 'nr', 'nf', 'mm', 'kp', 'cs', 'ax')";
