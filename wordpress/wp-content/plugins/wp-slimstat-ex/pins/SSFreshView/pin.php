...
class SSFreshView extends SSPins {
	...
	function _displayPanel() {
		...
		$SlimCfg->get['fi_encode'] .= '&amp;freshviewtb='.((!empty($_GET['freshviewtb']))?$_GET['freshviewtb']:'all');
		$_tb = (isset($_GET['freshviewtb']) && !empty($_GET['freshviewtb']))?$_GET['freshviewtb']:'all';
		$html = '';
	...
	function switchTable() {
		...
		// All
		$filter_url = '&amp;freshviewtb=all';
		$output .= "<a href=\"".$href.(($use_ajax)?'':$filter_url)."\" title=\"".__('View chart for &#039;All stats&#039;', 'wp-slimstat-ex')."\" ";
		$output .= "onclick=\"SlimStat.panel('".$pinid."', '".$filter_url."'); return false;\">";
		$output .= __('All', 'wp-slimstat-ex').$filter_img."</a> | ";
		// Common
		$filter_url = '&amp;freshviewtb=common';
		$output .= "<a href=\"".$href.(($use_ajax)?'':$filter_url)."\" title=\"".__('View chart for &#039;common stats&#039;', 'wp-slimstat-ex')."\" ";
		$output .= "onclick=\"SlimStat.panel('".$pinid."', '".$filter_url."'); return false;\">";
		$output .= __('Common', 'wp-slimstat-ex').$filter_img."</a> | ";
		// Feed
		$filter_url = '&amp;freshviewtb=feed';
		$output .= "<a href=\"".$href.(($use_ajax)?'':$filter_url)."\" title=\"".__('View chart for &#039;feed stats&#039;', 'wp-slimstat-ex')."\" ";
		$output .= "onclick=\"SlimStat.panel('".$pinid."', '".$filter_url."'); return false;\">";
...
';
	...
	function getHTML_PastDay($_tb='') {
		if(empty($_tb) || $_tb == '(1 = 1)') $_tb = (empty($_GET['freshviewtb'])) ? 'all':$_GET['freshviewtb'];
/*		$file = 'caches/pastday_graph_'.$_tb.'.svg';
		...
		$moid =& $this->getMoID(0);
		return SSFunction::get_module_custom($moid, $html, 'wide');
...
/*		$file = 'caches/pastyear_graph_'.$_tb.'.svg';
	...
	function generateSVG_PastDay($_tb) {
		...
		$thisHourFormatted = date('H', $SlimCfg->sstime($dt_this_hour));
		$hvu = array();
		for ($i = 0; $i < 24; $i++) {
			$j = $dt_this_hour - ($i * 60 * 60);
			$hvu = SSFunction::calc_hvu( ( $j - 3599 ), $j, $_tb );
			$statsData[] = array( 'hour' => date('H' , $SlimCfg->sstime($j)),
				  'hits' => $hvu['hits'],
...
//		$this->saveFile($svgCachePath, $svgFile);
	...
	function generateSVG_PastWeek($_tb) {
		global $SlimCfg;
		$day = $SlimCfg->dt[1];// blog midnight as servertime
		$todayDay = date('D', $SlimCfg->sstime($day));
		// Past 7 days
		for ($i = 0; $i < 7; $i++) {
			$j = $day - ($i * 60 * 60 * 24);
			$hvu = SSFunction::calc_hvu( $j, ($j + 86399), $_tb);
			$statsData[] = array( 'day' => date('D', $SlimCfg->sstime($j)),
				  'hits' => $hvu['hits'],
				  'uniques' => $hvu[$SlimCfg->option['visit_type']] );
		}
...
//		$this->saveFile($svgCachePath, $svgFile);
	...
	function generateSVG_PastMonth($_tb) {
		...
		for ($i = 0; $i < 29; $i++) {
			$j = $day - ($i * 60 * 60 * 24);
			$hvu = SSFunction::calc_hvu( $j, ($j + 86399), $_tb);
			$statsData[] = array( 'day' => date('M j', $SlimCfg->sstime($j)),
				  'hits' => $hvu['hits'],
				  'uniques' => $hvu[$SlimCfg->option['visit_type']] );
		}
		...
		// Weekend Highlighting
		$todayDay = date('D', $SlimCfg->dt[0]);//blog midnight as blogtime
		for ($i = 1; $i <= 5; $i++) {
...
//		$this->saveFile($svgCachePath, $svgFile);
	...
	function generateSVG_PastYear($_tb) {
		...
		for ($i = 0; $i < 12; $i++) {
			...
			$statsData[] = array('month' => date('M', $SlimCfg->sstime($dt_start)),
				  'hits' => $hvu['hits'],
				  'uniques' => $hvu[$SlimCfg->option['visit_type']] );
			$dt_end = $dt_start - 1;
