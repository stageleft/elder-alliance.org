...
Description ::
	...
	You can check displayable module list on your SlimStat-Admin page
*/
...
<tr valign="top"> 
	<th width="20%" scope="row"><?php _e('Use Content Filter:', 'wp-slimstat-ex') ?></th> 
	<td><?php _e('Replace {SLIMSTAT|1,2,3...} on content with displayable modules.', 'wp-slimstat-ex'); ?><br />
	<?php printf(__('Displayable modules are available at <a href="%s">SlimStat-Admin page</a>.', 'wp-slimstat-ex'), $SlimCfg->pluginURL.'/lib/ss-admin/modulelist.php'); ?><br />
	<?php _e('You can manually insert &lt;?php wpSSBlogStats(); ?&gt; on your template file', 'wp-slimstat-ex'); ?><br />
...
<tr valign="top"> 
	<th width="20%" scope="row"><?php _e('AJAX Library:', 'wp-slimstat-ex') ?></th> 
	<td><?php _e('Select javascript library for AJAX features or NONE for no AJAX', 'wp-slimstat-ex'); ?><br />
...
<?php
	...
	function wp_head() {
		global $SlimCfg;
		echo "\n".'<link rel="stylesheet" type="text/css" media="screen" href="'.$SlimCfg->pluginURL.'/pins/SSBlogStats/blogstats.css" />';
	}
...
//		if(!$this->op['ajax_lib'] || !function_exists('wp_print_scripts')) // only support AJAX with script-loader.php
		...
		$wrapper = '<a id="ajax_request" href="'.$SlimCfg->ajaxReq.'"></a>';
		$wrapper .= '<a id="slm2" name="slm2"></a>'."\n";
		$wrapper .= '<div id="wp_slimstat">'."\n";
		$nav = '<p class="module_nav">'.__('Modules', 'wp-slimstat-ex').' : ';
		$output = '';
		...
		foreach($IDs as $id) {
			...
			if($module) {
				$output .= call_user_func(array('SSModule', $module), '(1 = 1)');
				...
				$output .= call_user_func(array(&$temp_pin, $pin["modules"][$mo]["name"]), '(1 = 1)');
	/*			eval('$'.$pin["name"].' =& new $pin["name"]();'."\n");
				eval('$output .= $'.$pin["name"].'->'.$pin["modules"][$mo]["name"].'(\'(1 = 1)\');'."\n");*/
			}
		}
